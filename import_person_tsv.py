...
def import_tsv_to_person(tsv_file):
    ...
    df = pd.read_csv(
        tsv_file, sep="\t", dtype=str
    )  # Read everything as string from the start
    ...
    with psycopg.connect(TARGET_URL) as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                WHERE table_schema = 'omop_nlp' AND table_name = 'person'
            ...
            if not matched_columns:
                print(
                    "Error: No TSV columns match any columns in omop_nlp.person. Aborting."
                )
                sys.exit(1)

            if missing_in_db:
                print(
                    f"  Warning: These TSV columns don't exist in omop_nlp.person and will be skipped: {missing_in_db}"
                )
            if missing_in_tsv:
                print(
                    f"  Note: These omop_nlp.person columns are not in the TSV (DB defaults will apply): {missing_in_tsv}"
                )
            ...
            buffer = io.StringIO()
            df_to_insert.to_csv(
                buffer, sep="\t", index=False, header=False, na_rep="\\N"
            )
            buffer.seek(0)

            print(
                f"Inserting {len(df_to_insert)} rows into omop_nlp.person via COPY..."
            )
            with cursor.copy(
                f"COPY omop_nlp.person ({col_list}) FROM STDIN WITH (FORMAT text, DELIMITER E'\\t', NULL '\\N')"
            ) as copy:
