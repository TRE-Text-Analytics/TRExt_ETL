...
def import_tsv_to_person(tsv_file):
    ...
    with psycopg.connect(TARGET_URL) as conn:
        with conn.cursor() as cursor:
            ...
            # Fetch the actual columns in omop.person
            cursor.execute("""
            ...
            if not matched_columns:
                print("Error: No TSV columns match any columns in omop.person. Aborting.")
                sys.exit(1)
 
            if missing_in_db:
                print(f"  Warning: These TSV columns don't exist in omop.person and will be skipped: {missing_in_db}")
            if missing_in_tsv:
                print(f"  Note: These omop.person columns are not in the TSV (DB defaults will apply): {missing_in_tsv}")
            ...
            print(f"Inserting {len(df_to_insert)} rows into omop.person via COPY...")
            with cursor.copy(f"COPY omop.person ({col_list}) FROM STDIN WITH (FORMAT text, DELIMITER E'\\t', NULL '\\N')") as copy:
                copy.write(buffer.read())
 
            conn.commit()
 
    print(f"Done! {len(df_to_insert)} rows successfully appended to omop.person.")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python import_to_omop.py <file.tsv>")
        sys.exit(1)
